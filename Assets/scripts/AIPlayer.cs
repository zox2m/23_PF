...
public class AIPlayer
{
	...
	Brain brain;
	FloorCardManager floor_card_manager;

    LocalServer local_server;

    public AIPlayer(LocalServer local_server)
    {
        this.local_server = local_server;
		this.brain = new Brain();
		this.hand_cards = new List<Card>();
		this.floor_card_manager = new FloorCardManager();
		reset();
    ...
    {
        CPacket clone = LocalServer.pre_send(msg);
        CPacket.destroy(msg);
        on_receive(clone);
    }
	...
	void update_floor_cards(CPacket msg)
	{
		...
		for (byte i = 0; i < slot_count; ++i)
		{
			...
			for (byte card_index = 0; card_index < card_count; ++card_index)
			{
				...
				this.floor_card_manager.puton_card(new Card(number, pae_type, position));
			}
		}
	}
				...
				case CARD_EVENT_TYPE.BOMB:
					{
						...
						for (byte i = 0; i < bomb_card_count; ++i)
						{
							...
							Card card = this.hand_cards.Find(obj => obj.is_same(number, pae_type, position));
