...
public class PlayerAgent
{
	...
	public Card pop_card_from_hand(
		byte card_number,
		PAE_TYPE pae_type,
		byte position)
    {
	...
	public List<Card> pop_card_from_floor(byte pee_count_to_want)
	{
		...
		else
		{
			...
			{
				Card onepee_card = player_pees.Find(obj => obj.status != CARD_STATUS.TWO_PEE);
				if (onepee_card != null)
				{
		...
		switch (pae_type)
		{
			...
			case PAE_TYPE.KWANG:
				...
				else if (cards.Count == 3)
				{
					...
					bool is_exist_beekwang = cards.Exists(obj => obj.is_same_number(Card.BEE_KWANG));
