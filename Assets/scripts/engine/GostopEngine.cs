...
public class GostopEngine
{
	...
	public FloorCardManager floor_manager { get; private set; }
	...
	{
		List<Card> bonus_cards = this.floor_manager.pop_bonus_cards();
		...
		byte is_shaking)
	{
		...
		Card card = this.player_agents[player_index].pop_card_from_hand(
			card_number, pae_type, position);
		if (card == null)
		{
	...
	public List<Card> get_random_cards(int n)
	{
		List<Card> clone_cards = new List<Card>();
