...
public class PlayRoomUI : SingletonMonobehaviour<PlayRoomUI>, IMessageReceiver {
	...
	[SerializeField]
	PlayerCardPosition[] player_card_positions;
	...
	void Awake()
	{
		...
		EffectManager.Instance.load_effects();

		this.waiting_packets = new Queue<CPacket>();
		this.card_collision_manager = GameObject.Find("GameManager").GetComponent<CardCollision>();
		...
		this.player_info_slots.Add(transform.Find("player_info_01").GetComponent<PlayerInfoSlot>());
		this.player_info_slots.Add(transform.Find("player_info_02").GetComponent<PlayerInfoSlot>());

		CPacketBufferManager.initialize(1);
		this.back_image = SpriteManager.Instance.get_sprite("back");
	...
	{
		SpriteLayerOrderManager.Instance.reset();
		Vector3 pos = this.deck_slot.position;
		...
		for (int i = 0; i < this.total_card_pictures.Count; ++i)
		{
			...
			this.total_card_pictures[i].sprite_renderer.sortingOrder = 
				SpriteLayerOrderManager.Instance.Order;
		}
	}
	...
	Sprite get_hwatoo_sprite(Card card)
	{
		...
		return SpriteManager.Instance.get_card_sprite(sprite_index);
	}


	IEnumerator distribute_cards(Queue<Card> floor_cards, Dictionary<byte, Queue<Card>> player_cards)
	{
		...
		for (int looping = 0; looping < 2; ++looping)
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				CardPicture card_picture = this.deck_cards.Pop();
	...
	void sort_floor_cards_after_distributed(List<CardPicture> begin_cards_picture)
	{
		...
		for (byte i = 0; i < begin_cards_picture.Count; ++i)
		{
			...
			VisualFloorSlot slot = this.floor_ui_slots.Find(obj => obj.is_same_card(number));
	...
	void sort_floor_cards_when_finished_turn()
	{
		for (int i = 0; i < this.floor_ui_slots.Count; ++i)
		{
			VisualFloorSlot slot = this.floor_ui_slots[i];
	...
	public void enter()
	{
		...
		NetworkManager.Instance.message_receiver = this;
        NetworkManager.Instance.start_localserver();
		...
		while (true)
		{
			...
			switch (protocol)
			{
				...
				case PROTOCOL.PLAYER_ORDER_RESULT:
					{
						...
						UIManager.Instance.show(UI_PAGE.POPUP_PLAYER_ORDER);
						PopupPlayerOrder popup =
							UIManager.Instance.get_uipage(UI_PAGE.POPUP_PLAYER_ORDER).GetComponent<PopupPlayerOrder>();
						...
						GameObject ef = UIManager.Instance.get_uipage(UI_PAGE.POPUP_FIRST_PLAYER);
						if (head == 0)
						{
						...
						UIManager.Instance.show(UI_PAGE.POPUP_FIRST_PLAYER);

						yield return new WaitForSeconds(1.5f);
						UIManager.Instance.hide(UI_PAGE.POPUP_PLAYER_ORDER);
						UIManager.Instance.hide(UI_PAGE.POPUP_FIRST_PLAYER);
					}
					break;
				...
				case PROTOCOL.START_TURN:
					{
						...
						if (this.is_test_mode)
						{
							...
							CardPicture card_pic = this.player_hand_card_manager[0].get_card(0);
							...
							NetworkManager.Instance.send(card_msg);
						}
						else
						{
				...
				case PROTOCOL.ASK_GO_OR_STOP:
					UIManager.Instance.show(UI_PAGE.POPUP_GO_STOP);
					break;
				...
				case PROTOCOL.ASK_KOOKJIN_TO_PEE:
					UIManager.Instance.show(UI_PAGE.POPUP_ASK_KOOKJIN);
					break;
	...
	{
		UIManager.Instance.show(UI_PAGE.POPUP_GO_COUNT);
		UIManager.Instance.get_uipage(UI_PAGE.POPUP_GO_COUNT).GetComponent<PopupGo>().refresh(count);

		yield return new WaitForSeconds(1.0f);
		UIManager.Instance.hide(UI_PAGE.POPUP_GO_COUNT);
	}
	...
	IEnumerator on_game_result(CPacket msg)
	{
		...
		UIManager.Instance.show(UI_PAGE.POPUP_STOP);
		yield return new WaitForSeconds(2.0f);

		UIManager.Instance.hide(UI_PAGE.POPUP_STOP);

		UIManager.Instance.show(UI_PAGE.POPUP_GAME_RESULT);
		PopupGameResult popup = 
			UIManager.Instance.get_uipage(UI_PAGE.POPUP_GAME_RESULT).GetComponent<PopupGameResult>();
	...
	List<Card> parse_cards_to_get(CPacket msg)
	{
	...
	List<CardPicture> parse_cards_to_take_from_others(byte player_index, CPacket msg)
	{
		// 뺏어올 카드.
		...
		for (byte victim = 0; victim < victim_count; ++victim)
		{
			...
			for (byte i = 0; i < count_to_take; ++i)
			{
				...
				CardPicture card_pic = this.player_card_manager[victim_index].get_card(
	...
	IEnumerator on_turn_result(byte player_index, CPacket msg)
	{
		...
		List<CardPicture> take_cards_from_others = parse_cards_to_take_from_others(player_index, msg);
	...
	IEnumerator on_select_card_ack(CPacket msg)
	{
		...
		{
			UIManager.Instance.show(UI_PAGE.POPUP_SHAKING_CARDS);
			PopupShakingCards popup =
				UIManager.Instance.get_uipage(UI_PAGE.POPUP_SHAKING_CARDS).GetComponent<PopupShakingCards>();
			...
			yield return new WaitForSeconds(1.5f);
			UIManager.Instance.hide(UI_PAGE.POPUP_SHAKING_CARDS);
		}
		...
		if (card_event != CARD_EVENT_TYPE.NONE)
		{
			...
			if (card_event != CARD_EVENT_TYPE.SHAKING)
			{
				EffectManager.Instance.play(card_event);
				yield return new WaitForSeconds(1.5f);
			}
		}
		...
		if (player_index == this.player_me_index)
		{
			...
			if (select_result == PLAYER_SELECT_CARD_RESULT.CHOICE_ONE_CARD_FROM_PLAYER)
			{
				UIManager.Instance.show(UI_PAGE.POPUP_CHOICE_CARD);
				PopupChoiceCard popup =
					UIManager.Instance.get_uipage(UI_PAGE.POPUP_CHOICE_CARD).GetComponent<PopupChoiceCard>();
			...
			else
			{
				...
				NetworkManager.Instance.send(flip_msg);
			}
		}
	}
	...
	IEnumerator on_flip_deck_card_ack(CPacket msg)
	{
		...
		else
		{
			...
			if (events.Count > 0)
			{
				...
				{
					EffectManager.Instance.play(events[i]);
					yield return new WaitForSeconds(1.5f);
				}
			}
	...
	IEnumerator move_after_flip_card(byte player_index,
		List<CardPicture> take_cards_from_others,
		List<Card> cards_to_give)
	{
		...
		for (int i = 0; i < cards_to_give.Count; ++i)
		{
			...
			CardPicture card_pic = slot.find_card(cards_to_give[i]);
		...
		NetworkManager.Instance.send(finish);
	}


	IEnumerator flip_deck_card(CardPicture deck_card_picture)
	{
		...
		byte player_card_position)
	{
		...
		if (event_type == CARD_EVENT_TYPE.BOMB)
		{
			...
			if (this.player_me_index == player_index)
			{
				...
				{
					CardPicture card_picture = this.player_hand_card_manager[player_index].find_card(
			...
			else
			{
				for (int i = 0; i < bomb_cards_info.Count; ++i)
				{
					CardPicture card_picture = this.player_hand_card_manager[player_index].get_card(i);
		...
		else
		{
			...
			CardPicture card_picture = this.player_hand_card_manager[player_index].get_card(slot_index);
			targets.Add(card_picture);
		...
		if (event_type == CARD_EVENT_TYPE.BOMB)
		{
			...
			EffectManager.Instance.play_dust(to, 0.3f, true);
		}
	...
	{
		card_picture.sprite_renderer.sortingOrder = SpriteLayerOrderManager.Instance.Order;
	...
	void move_card_to_floor(CardPicture card_picture, CARD_EVENT_TYPE event_type)
	{
		byte slot_index = 0;
		...
		else
		{
			...
			if (event_type != CARD_EVENT_TYPE.BOMB)
			{
				EffectManager.Instance.play_dust(to, 0.1f, false);
			}
	...
	{
		VisualFloorSlot slot = this.floor_ui_slots.Find(obj => obj.get_card_count() == 0);
		if (slot == null)
		{
	...
	{
		PlayerHandCardManager hand_card_manager = this.player_hand_card_manager[player_index];
	...
	void refresh_player_floor_slots(PAE_TYPE pae_type, byte player_index)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			CardPicture card_pic = this.player_card_manager[player_index].get_card_at(pae_type, i);
	...
	void on_card_touch(CardPicture card_picture)
	{
		...
		else
		{
			...
			{
				UIManager.Instance.show(UI_PAGE.POPUP_ASK_SHAKING);
				PopupShaking popup =
					UIManager.Instance.get_uipage(UI_PAGE.POPUP_ASK_SHAKING).GetComponent<PopupShaking>();
				popup.refresh(card_picture.card, card_picture.slot);
			}
			else
			{
				PlayRoomUI.send_select_card(card_picture.card, card_picture.slot, 0);
			}
		}
	}
	...
	// 힌트 화살표.
	GameObjectPool<GameObject> hint_arrows;
	...
	void load_hint_arrows()
	{
		...
		this.hint_arrows = new GameObjectPool<GameObject>(10, arrow, (GameObject original) =>
		{
	...
	public void refresh_hint_mark()
	{
		...
		{
			CardPicture card_picture = this.player_hand_card_manager[this.player_me_index].get_card(i);
			VisualFloorSlot slot =
				this.floor_ui_slots.Find(obj => obj.is_same_card(card_picture.card.number));
			if (slot == null)
			{
	...
	{
		bool option_hint = 
			UIManager.Instance.get_uipage(UI_PAGE.GAME_OPTION).GetComponent<GameOption>().is_hint_on();
