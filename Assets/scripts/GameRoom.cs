...
public class GameRoom
{
	...
	PlayerOrderManager order_manager;

    public GameRoom(LocalServer server)
    {
		this.server = server;
        this.engine = new GostopEngine();
        this.players = new List<Player>();
        this.received_protocol = new Dictionary<byte, PROTOCOL>();
		this.order_manager = new PlayerOrderManager();
    }


    public void add_player(Player newbie)
    {
	...
	void broadcast_go_count(Player player)
	{
	...
	void next_turn()
	{
		...
		// 바닥 카드 갱신을 위한 데이터.
		List<FloorSlot> slots = this.engine.floor_manager.slots;
		...
		for (int i = 0; i < slots.Count; ++i)
		{
			...
			{
				Card card = slots[i].cards[card_index];
	...
	public void on_ready_req()
	{
		...
		for (int i = 0; i < this.players.Count; ++i)
		{
			...
			{
				Card card = this.order_manager.random_cards[slot_index];
				msg.push(slot_index);
    ...
    void send_cardinfo_to_player(Player player)
    {
	...
	{
		PlayerAgent target_player = this.players[player_index].agent;
		for (int i = 0; i < this.players.Count; ++i)
		{
    ...
    /// <returns></returns>
    byte get_aiplayer_delay(Player current_player)
    {
        byte delay = 0;
